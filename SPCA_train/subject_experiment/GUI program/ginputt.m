function [out1,out2,out3] = ginputt(arg1, usrFcn)
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    initialState = setupFcn(fig, usrFcn);
...
function initialState = setupFcn(fig, usrFcn)
...
set(fig,'WindowButtonMotionFcn',@(o,e) ginputWindowButtonMotion(o,e, initialState.CrossHair, usrFcn));
...
function ginputWindowButtonMotion(fig, ~, crossHair, usrFcn)
% Manages the simulated display of the full cross hair pointer
updateCrossHair(fig, crossHair, usrFcn);
end

function updateCrossHair(fig, crossHair, usrFcn)
global state;
...
% % % thickness = 3; % 1 Pixel thin lines. 
...
% % % set(crossHair(1), 'Position', [cp(1)-gap+thickness/2 cp(2) 2*gap thickness]);
% % % set(crossHair(2), 'Position', [cp(1)-gap+thickness/2 cp(2) 2*gap thickness]);
% % % % set(crossHair(2), 'Position', [cp(1)+gap-thickness/2 cp(2) gap thickness]);
% % % set(crossHair(3), 'Position', [cp(1) cp(2)-gap+thickness/2 thickness 2*gap]);
% % % set(crossHair(4), 'Position', [cp(1) cp(2)-gap+thickness/2 thickness 2*gap]);
% % % % set(crossHair(4), 'Position', [cp(1) cp(2)+gap-thickness/2 thickness gap]);
% % % % usrFcn();
end
